...
void TradplusLoadBannerAd(const char* adUnitId, bool closeAutoShow, float x, float y, float width, float height,int adPosition,int contentMode, const char* sceneId, const char* customMap,const char* className,const char* localParams,bool openAutoLoadCallback, float maxWaitTime)
{
    NSString *jsonString = stringFromUTF8String(customMap);
    NSDictionary *dic = nil;
    if(jsonString != nil)
...
void TradplusLoadNativeBannerAd(const char* adUnitId, bool closeAutoShow, float x, float y, float width, float height,int adPosition, const char* sceneId, const char* customMap,const char* className,const char* localParams,bool openAutoLoadCallback, float maxWaitTime)
{
    NSString *jsonString = stringFromUTF8String(customMap);
    NSDictionary *dic = nil;
    if(jsonString != nil)
    ...
    NSString *sceneIdStr = stringFromUTF8String(sceneId);
    NSString *classNameStr = stringFromUTF8String(className);
    [[TPUNativeBannerManager sharedInstance] loadWithAdUnitID:adUnitIDStr closeAutoShow:closeAutoShow x:x y:y width:width height:height adPosition:adPosition sceneId:sceneIdStr customMap:dic className:classNameStr localParams:localParamsDic openAutoLoadCallback:openAutoLoadCallback maxWaitTime:maxWaitTime];
}
...
void TradplusLoadSplashAd(const char* adUnitId,const char* customMap,const char* localParams)
{
    NSString *jsonString = stringFromUTF8String(customMap);
    NSDictionary *dic = nil;
    if(jsonString != nil)
    ...
    NSString *adUnitIDStr = stringFromUTF8String(adUnitId);
    [[TPUSplashManager sharedInstance] loadWithAdUnitID:adUnitIDStr customMap:dic localParams:localParamsDic];
}
